//! Texture-pack *loading*: unzip a pack of named PNGs (or slice the game's own atlas image), build
//! the grayscale modulation variants, and assemble the one-tile-wide atlas the 3D views sample.
//!
//! The atlas *layout contract* — `TexturePack`, `BLOCK_FACE_TEX`, `face_tile`, `face_color_and_row`,
//! `TILE` — lives in the shared `voxel-core` crate (plan PR-C), because `geometry` renders against
//! it and that layer is `tauri`-free. It is re-exported here so every `texturepack::…` call site in
//! this crate is unchanged; only the image/zip decoding below is app-side.
pub use voxel_core::texture::{TexturePack, TILE};
#[cfg(test)]
pub use voxel_core::texture::{face_color_and_row, BLOCK_FACE_TEX};
