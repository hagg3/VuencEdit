...
⚠️ **Removed from this repo:** OBJ (`export_obj`), JSON (`export_json`), and
MagicaVoxel VOX (`export_vox`) static-geometry export, and Source Engine VMF
export (`vmf_export.rs`, `export_vmf`/`estimate_vmf`) — a selection-to-Hammer-
brushwork converter with a 3D greedy box merge, Dev/Flat-color texture modes,
and an opt-in skybox auto-shell. All four are being reimplemented in the sibling
`~/EdenToMC` project instead; `vmf_export.rs`/`export.rs`/`colors.rs` were copied
there verbatim as a starting point before deletion here. The `MAX_EXPORT_VOXELS`/
`check_export_volume` volume guard those four exporters shared (256 M voxels,
matching `MAX_CLIPBOARD_VOLUME`) went with them — PNG never needed it, since it
renders the whole map at a fixed resolution regardless of selection size.
